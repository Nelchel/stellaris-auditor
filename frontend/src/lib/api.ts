...
export async function getHistory(): Promise<any> {
  ...
  try {
    ...
    if (!payload || !payload.series || !payload.series.labels) {
      return { ...demoHistory, source: 'demo-fallback-empty-response' }
    }

    if ((payload.count ?? 0) === 0) {
      return { ...demoHistory, source: 'demo-fallback-no-local-reports' }
    }

    return { ...payload, source: 'local-backend' }
  } catch (error) {
    return { ...demoHistory, source: 'demo-fallback-backend-unavailable' }
  }
}
